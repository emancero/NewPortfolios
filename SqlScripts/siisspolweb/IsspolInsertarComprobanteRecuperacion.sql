...
begin
	...
	begin tran
		...
		IF NOT EXISTS (
			SELECT 1 FROM bvq_backoffice.ISSPOL_RECUPERACION
			WHERE ISR_NUMERACION=@i_nombre
			--AND datediff(hh,ISR_FECHA,@i_fecha)=0
			AND convert(varchar,ISR_FECHA,20)=convert(varchar,@i_fecha,20)
		)
		BEGIN
