...
-- Description: Obtiene el detalle y saldos de todos los títulos del portafolio para master-grid control de ing/egr
-- Parametros:  
-- Historial:   
-- =============================================
                        ...
                        FROM BVQ_BACKOFFICE.TITULOS_PORTAFOLIO AS TPOR 
                             ...
                             WHERE /*(
								 ...
								 @i_por_id is null
                                         ...
                                         AND*/ HTP.HTP_ESTADO<>@v_id_estado 
                                         AND HTP.HTP_ESTADO<>@v_id_cancelado
                                         --AND isnull(TIT.TIV_FECHA_VENCIMIENTO,@i_fecha)>=@i_fecha
										 AND datediff(d,TPOR.TPO_FECHA_INGRESO,@i_fecha)=0
										 --AND 1=0
            ) s
		...
		distinct TPOR.por_id/TPOR.por_id* /*newPors  @i_por_id/@i_por_id*/
						...
						TPO_SALDO=(select sum(montooper-isnull(remaining,0)) from bvq_backoffice.eventoportafolio e
							...
							where (
								...
								tpo.tpo_numeracion=tpor.tpo_numeracion and tpo.tiv_id=tpor.tiv_id
								--or
						...
						TPO_ID =
							TPOR.TPO_ID,
							--(select max(htp_id) from bvq_backoffice.historico_titulos_portafolio where tpo_id_c=TPOR.TPO_ID),
