...
begin
	...
	from(
		SELECT row_number() over (
			partition by tpo_id_c,c.c ORDER BY CASE WHEN PAR.PAR_VALOR='SI' THEN 1 ELSE -1 END
			* datediff(d,0,HTP_FECHA_OPERACION) asc, CASE WHEN PAR.PAR_VALOR='SI' THEN 1 ELSE -1 END * htp_id asc
		) r
		, htp_precio_compra,h.tiv_id,tpo_id_c,c cc
