...
BEGIN
	...
	begin
		...
		if @i_todos_los_vigentes=0
			...
			select
			 Errores=
			 case when Tipo_Instrumento not in (4) and isnull(fecha_ultima_calificacion,0)=0 then
				'Sin calificación y no es bono.' else '' end
			+case when isnull(Casa_de_Valores_codigo,'')='' then
				'Sin casa de valores'
			 end
			,EMS_NOMBRE
			,FON_ID
...
join [192.168.2.225].sicav.bvq_backoffice.fondo fon on fon.fon_id=e.fon_id
...
group by fecha_compra,left(fon_numeracion,3)
