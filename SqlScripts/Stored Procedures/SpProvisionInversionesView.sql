...
BEGIN
	...
	; with  cte_provision as (
        ...
        END
			  ...
			  FROM BVQ_BACKOFFICE.PortafolioCorte pfc INNER JOIN BVQ_ADMINISTRACION.TIPO_VALOR tvd
			  ...
			  GROUP BY doc.orden, seg.orden,DOC.Codigo, seg.por_id, DOC.NOMBRE, doc.etiqueta, seg.nombre_contabilidad,pfc.TVL_CODIGO, DOC.NOMBRE_CONTABILIDAD,tvd.TVL_NOMBRE, pfc.sector_general, pfc.por_codigo, seg.nombre
			 -- order by doc.orden
        ...
        END
		 ...
		 select orden_doc, orden_seg, Codigo1, por_id1, titulo, nombre_etiqueta,sum(distribucion)  as distribucion, sum(TOTAL_PROVISION) provision 
		 into #CTE
		 from cte_provision
		 group by orden_doc, orden_seg, Codigo1, por_id1, titulo, nombre_etiqueta
		...
		cte.orden_seg, 
		(per1.acreedora /*+ '.' + per1.acreedoraAux*/) as cuenta1,  
		(per2.acreedora /*+ '.' + per2.acreedoraAux*/) as cuenta2,
		cte.TITULO,
		cte.nombre_etiqueta, 
		...
		group by cte.orden_doc, cte.orden_seg, per1.acreedora,  per2.acreedora, per1.acreedoraAux, per2.acreedoraAux, cte.TITULO,cte.nombre_etiqueta, cte.distribucion, cte.provision
		order by orden_doc asc, orden_seg asc 
