CREATE view bvq_backoffice.IsspolRentaFijaViewNew as
  select
...
VALNOM_ANTERIOR=VALNOM_ANTERIOR,    
 ...
 -- SECTOR_DETALLADO=case when max(SECTOR_GENERAL)='SEC_PRI_FIN' then
  ...
  PRECIO_LIMPIO=max(s.PRECIO_LIMPIO)
     --,max(eca_nombre),max(eca_fecha_resolucion)--cs=sum(checksum(eca_nombre,ECA_FECHA_RESOLUCION)%1000)
  from(
   select    
   TVL_NOMBRE=
...
ABONO_INTERES=TPO_ABONO_INTERES,    
   ...
   ,PRECIO_LIMPIO=pc.PRECIO_DE_HOY
  ...
  join BVQ_BACKOFFICE.PORTAFOLIO port on pc.por_id = port.POR_ID  
    ...
    (    
     ...
     select
	  isnull(lead(eca_fecha_resolucion) over (partition by emi_id order by eca_fecha_resolucion,eca_id),'99991231') eca_fecha_resolucion_hasta
	 ,isnull(lead(eca_id) over (partition by emi_id order by eca_fecha_resolucion,eca_id),2147483647) eca_id_hasta
	 ...
	 --and (eca_fecha_resolucion is null or eca_fecha_resolucion<=(select c from corteslist))    
    ) emical on emical.emi_id=tiv_emisor and
	tfcorte>=isnull(eca_fecha_resolucion,0) and tfcorte<eca_fecha_resolucion_hasta
	--emical.r=1--(tvl_generico=1 or tiv_tipo_valor in (/*10,*/13)) and emical.emi_id=tiv_emisor and emical.r=1  

    left join    
    (    
    
     select
	 isnull(lead(enc_fecha_desde) over (partition by enc.enc_numero_corto_emision order by enc.enc_fecha_desde,enc.enc_id),'99991231') enc_fecha_desde_hasta
	 ,isnull(lead(enc_id) over (partition by enc.enc_numero_corto_emision order by enc.enc_fecha_desde,enc.enc_id),2147483647) enc_id_hasta
	 
	 ,row_number() over (partition by enc.enc_numero_corto_emision order BY enc.ENC_FECHA_DESDE  desc,enc.ENC_ID desc) r,enc_numero_corto_emision    
     ,enc.ENC_VALOR 
     ...
     where enc.ENC_ESTADO=21
	 --and (enc.ENC_FECHA_DESDE is null or enc.ENC_FECHA_DESDE<=(select c from corteslist))    
    ) emscal on emscal.enc_numero_corto_emision=pc.TIV_CODIGO_TITULO_SIC and
	tfcorte>=isnull(enc_fecha_desde,0) and tfcorte<enc_fecha_desde_hasta
	--emscal.r=1  
